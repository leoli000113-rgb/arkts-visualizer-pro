/**
 * IR 数据模型 —— 从 Web 版 app/src/ir/types.ts 移植（ArkTS 严格模式适配）
 * 与 Web 版保持逐字段一致；等价性由 app/src/native/nativePort.test.ts 保证。
 */
...
/** struct 成员：状态声明（结构化）| 原文块（方法/字段）| @Builder（签名原文 + 方法体结构化）| build() 标记位 */
